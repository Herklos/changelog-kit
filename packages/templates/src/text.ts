...
const DEFAULT_TOKEN_STYLE: Record<string, Record<string, unknown>> = {
  bold: { fontWeight: '700' },
...
export interface RichTextProps {
  ...
  tokenStyle?: Record<string, Record<string, unknown>>;
}
 ...
 */
export function RichText({ value = '', style, tokenStyle = {} }: RichTextProps): ReactElement {
  ...
  const inheritedFontSize = typeof style?.fontSize === 'number' ? (style.fontSize as number) : undefined;
  const defaultTokenStyle: Record<string, Record<string, unknown>> = {
    ...DEFAULT_TOKEN_STYLE,
    code: {
      ...DEFAULT_TOKEN_STYLE.code,
      ...(inheritedFontSize != null ? { fontSize: inheritedFontSize * 0.92 } : null)
    }
  };
  const style2: Record<string, Record<string, unknown>> = { ...defaultTokenStyle, ...tokenStyle };
