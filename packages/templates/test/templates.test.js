...
test('layouts and shared components hardcode no color beyond the onDark fallback', () => {
  ...
  const files = [
    ...
    ...['components.js', 'gradients.js', 'canvas.js', 'image.js', 'text.js', 'theme.js'].map((f) => `${srcDir}${f}`)
  ];
...
// import would break the Node/SSR path (see rn.web.js's comment).
