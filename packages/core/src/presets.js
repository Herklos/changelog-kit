...
/** @typedef {keyof typeof SIZE_PRESETS} SizePresetName */

/**
 * Resolve a target into a concrete canvas size.
 * @param {import('./types.js').Target} target
 * @param {import('./types.js').Template} [template]
 * @returns {import('./types.js').RenderSize}
 */
export function resolveSize(target, template) {
  const preset = target.preset ? SIZE_PRESETS[target.preset] : undefined;
  if (target.preset && !preset) {
    throw new Error(
      `Unknown size preset "${target.preset}". Known: ${Object.keys(SIZE_PRESETS).join(', ')}`
    );
  }
