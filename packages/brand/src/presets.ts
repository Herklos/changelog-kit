...
export const octobotDark: BrandKit = defineBrandKit({
  id: 'octobot-dark',
  name: 'OctoBot — Dark',
  colors: {
    canvas: '#0f1237',
  ...
  imagery: {
    style:
      'clean 3D render, friendly stylised octopus mascot wearing the OctoBot helmet, glossy turquoise and frosted-blue materials, trading charts and candlesticks, soft studio lighting',
...
export const octobotLight: BrandKit = defineBrandKit({
  extends: octobotDark,
  id: 'octobot-light',
  name: 'OctoBot — Light',
  colors: {
...
export const octobotHalo: BrandKit = defineBrandKit({
  extends: octobotDark,
  id: 'octobot-halo',
  name: 'OctoBot — Halo',
  colors: {
...
export const midnight: BrandKit = defineBrandKit({
...
export const paper: BrandKit = defineBrandKit({
  id: 'paper',
  name: 'Paper',
  colors: {
    canvas: '#ffffff',
...
export const brandPresets: Record<string, BrandKit> = { octobotDark, octobotLight, octobotHalo, midnight, paper };

export function getBrandPreset(id: string): BrandKit {
  const found = Object.values(brandPresets).find((b) => b.id === id) ?? brandPresets[id];
  if (!found) {
    throw new Error(`Unknown brand preset "${id}". Known: ${Object.values(brandPresets).map((b) => b.id).join(', ')}`);
  }
  return found;
}
