 ...
 * @property {'hero'|'card'|'background'} [role]
 * @property {number} [width]
 * @property {number} [height]
 * @property {string} [aspect]     e.g. "4:5"
 * @property {number} [n]
...
export class ImageProvider {
  /** @param {{id?:string, model?:string, defaults?:object}} [config] */
  constructor(config = {}) {
    this.id = config.id ?? this.constructor.providerId ?? 'provider';
  ...
  prepare(request) {
    const merged = { n: 1, ...this.defaults, ...request };
...
export function toDataUri(buffer, mime = 'image/png') {
