...
import type { ChangelogDoc, ChangelogEntry } from '@changelog-kit/core';
  ...
  return h(
    View,
    {
      style: {
        ...
        backgroundColor: theme.colors.badge[kind],
    ...
    h(
      Text,
      {
        style: {
          ...
          letterSpacing: u(0.6),
          color: theme.colors.badgeInk[kind]
        }
      },
      entry.badge
    )
...
/** Feature card: badge + title + body on one side, art on the other. Mirrors the HTML `card()` + `cardCss`. */
    ...
    ? h(ArtSlot, {
        ...
        style: center
          ...
          : {
              ...
              flexGrow: 0,
              flexShrink: 0,
              flexBasis: `${Math.round(artRatio * 100)}%`,
              alignSelf: 'stretch',
              minHeight: u(120)
            }
      })
...
export function VersionMark({ doc, theme, u, size = 260, label = '' }: VersionMarkProps): ReactElement {
