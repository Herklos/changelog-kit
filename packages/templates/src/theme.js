 ...
 * @param {import('@changelog-kit/brand').BrandKit} brand
 * @param {number} unit from `createScale()`
 * @param {{display?: string, body?: string}} [fontFamilies] host-supplied font family names —
 *   brand fonts are Google-loaded via a `<link>` on the web path (`@changelog-kit/brand`'s
 *   `fontHead`), which a native app cannot do; native callers must pass real family names
 *   (e.g. loaded with `expo-font`) or fall back to the system font.
 * @returns {{colors: object, shadow: object, radius: object, spacing: object, fonts: {display?: string, body?: string, scale: number}}}
 */
export function brandToTheme(brand, unit, fontFamilies = {}) {
  const scaleAll = (obj) => Object.fromEntries(Object.entries(obj ?? {}).map(([k, v]) => [k, v * unit]));
  return {
    ...
    shadow: brand.shadow,
    radius: scaleAll(brand.radius),
    spacing: scaleAll(brand.spacing),
    fonts: {
 ...
 * `<Changelog>` and `brand/src/fonts.js`) — RN native ignores CSS fallback lists.
 * @param {import('@changelog-kit/core').RenderContext & {baseWidth?: number, fontFamilies?: {display?: string, body?: string}}} ctx
 */
export function themeFromContext(ctx) {
