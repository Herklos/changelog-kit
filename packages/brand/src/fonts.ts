...
import type { BrandFont, BrandKit } from './brand-kit.js';

const GOOGLE = 'https://fonts.googleapis.com/css2';

function googleHref(fonts: BrandFont[]): string {
  const byFamily = new Map<string, Set<number>>();
  for (const font of fonts) {
    if (font?.source !== 'google') continue;
    const set = byFamily.get(font.family) ?? new Set<number>();
...
/** @returns markup to inject in <head> */
export function fontHead(brand: BrandKit): string {
  const fonts = [brand.fonts.display, brand.fonts.body].filter(Boolean) as BrandFont[];
  const out: string[] = [];
  ...
  const faces = fonts
    ...
    .map((f) =>
      (f.weights ?? [400])
        .map(
          (w) =>
            `@font-face{font-family:"${f.family}";font-weight:${w};font-display:block;` +
...
function fileUrl(path: string | undefined, weight: number): string {
  if (!path) return '';
...
export function usesWebfonts(brand: BrandKit): boolean {
