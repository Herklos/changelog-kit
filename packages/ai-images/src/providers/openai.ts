...
export interface OpenAIProviderConfig extends ImageProviderConfig {
  apiKey?: string;
  baseUrl?: string;
  quality?: string;
}
...
export class OpenAIImageProvider extends ImageProvider {
  ...
  async generate(request: ImageRequest): Promise<ImageResult> {
    ...
    const json = (await res.json()) as { data?: Array<{ revised_prompt?: string; b64_json?: string; url?: string }> };
