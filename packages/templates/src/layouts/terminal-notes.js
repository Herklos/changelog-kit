...
export const terminalNotes = {
  ...
  render(ctx) {
    ...
    const dot = (color) => h(View, { style: { width: u(14), height: u(14), borderRadius: 99, backgroundColor: color } });
              ...
              h(
                View,
                {
                  style: {
                    ...
                    borderRadius: u(9),
                    backgroundColor: theme.colors.badge[entry.kind] ?? 'rgba(255,255,255,.08)'
                  }
                },
                h(Text, { style: { fontWeight: '700', fontFamily: MONO, color: theme.colors.badgeInk[entry.kind] ?? onDark } }, SIGIL[entry.kind] ?? '*')
              ),
              h(
                Text,
