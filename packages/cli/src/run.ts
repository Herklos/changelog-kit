...
import type { Target, ImageProvider as CoreImageProvider } from '@changelog-kit/core';
...
export async function run(argv: string[]): Promise<void> {
  ...
  for (const size of sizes) {
    for (const format of formats) {
      const dims = /^\d+x\d+$/.test(size)
        ...
        : { preset: size };
      targets.push({
        format: format as Target['format'],
        ...dims,
  ...
  const kit = new ChangelogKit<BrandKit, Parameters<typeof toStaticHtml>[0]>({
    brand,
    templates: builtinTemplates,
    renderer: new PlaywrightRenderer({ baseUrl: pathToFileURL(path.dirname(path.resolve(docPath))).href + '/' }),
    serializer: toStaticHtml,
    // ai-images' `ImageProvider` is a deliberately core-agnostic shape (see
    // packages/ai-images/src/provider.ts) — the CLI is the one place allowed
    // to bridge it to core's `ImageProvider<TBrand>` interface.
    imageProvider: imageProvider as CoreImageProvider<BrandKit> | undefined,
    onEvent: (event, payload) => {
      if (event === 'render:done') {
        const p = payload as { template: string; target: { format: string }; size: { width: number; height: number } };
        console.log(`  ✓ ${p.template} ${p.size.width}x${p.size.height} ${p.target.format}`);
      }
      if (event === 'image:start') {
        const p = payload as { key: string };
        console.log(`  … generating image for ${p.key}`);
      }
    }
  });

  await mkdir(outDir, { recursive: true });
  const manifest: { file: string; width: number; height: number; scale?: number }[] = [];
  try {
    await kit.generate({
      doc,
      template: templates,
...
async function loadModuleOrJson(file: string): Promise<any> {
...
async function loadBrand(spec?: string): Promise<BrandKit> {
